...
from schemas.exceptions import AccessForbiddenException, EventAlreadyStartedException, \
    PaymentExpiredException, NotEnoughPlacesLeftException, CheckIsAlreadyPayedException
...
class CheckUseCase(AbstractCheckUseCase):
    ...
    async def create(self, user_id: int, data: CheckAddSchema):
        date = Dm.now()
        async with self.uow:
            check_id = await ChecksService.add_check(self.uow, user_id, data, Dm.date_to_string(date))
            cost = 0
    ...
    async def get_list_by_user(self, user_id: int, target_user_id: int, **filter_by):
        async with self.uow:
            if not await UsersService.user_is_moderator(self.uow, user_id):
                raise AccessForbiddenException
            checks_list = await ChecksService.get_checks_list(self.uow, user_id=target_user_id, **filter_by)

        return checks_list

    async def get_list_by_current_user(self, user_id: int, **filter_by):
        async with self.uow:
            checks_list = await ChecksService.get_checks_list(self.uow, user_id=user_id, **filter_by)
