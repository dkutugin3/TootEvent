...
def upgrade() -> None:
    ...
    op.alter_column('events', 'genre',
               existing_type=postgresql.JSON(astext_type=sa.Text()),
               nullable=False)
    op.alter_column('events', 'rating',
...
def downgrade() -> None:
    ...
    op.add_column('events', sa.Column('poster_path', sa.VARCHAR(), autoincrement=False, nullable=True))
    op.add_column('events', sa.Column('location', postgresql.JSON(astext_type=sa.Text()), autoincrement=False, nullable=True))
    op.alter_column('events', 'rating',
               existing_type=sa.INTEGER(),
               nullable=True)
    op.alter_column('events', 'genre',
